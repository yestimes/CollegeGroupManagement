    ...
    public static OrganizationBean getOrgaInfo(int o_id){
        ...
        try {
            if ((conn = DataSourceConfiguration.getConnection()) == null){
                System.out.println("datasource error, system will quit");
                System.exit(-1);
            }
            ...
            OrgaGmBean resset = OrgaGmListDao.query(o_id);
            bean.setO_name(resset.getO_name());
            ...
            String gm = OrgaGmListDao.OrgaGmName(o_id);
            bean.setGma_name(gm);
            String [] Ga = OrgaGmListDao.OrgaGaNames(o_id);
            bean.setGa_names(Ga);
